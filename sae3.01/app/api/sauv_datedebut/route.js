...
/**
 * Met à jour la date de début réelle d'une étape d'un chantier
 * @param {Request} req - La requête HTTP contenant l'ID du chantier, l'ID de l'étape et la date de début
...
export async function POST(req) {
  try {
    ...
    // Met à jour la date de début en utilisant la clé composite (chantier + étape)
    ...
    const updated = await prisma.etape_chantier.update({
      ...
      data: { 
        ...
        datedebut: parseDateISO(dateDebut) 
      },
    });
