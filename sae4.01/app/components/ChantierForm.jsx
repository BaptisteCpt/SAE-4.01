...
export default function ChantierForm() {
  ...
  const [adresse_du_chantier, setAdresseDuChantier] = useState(
    data?.adresse_du_chantier || "",
  );
  const [villechantier, setVilleChantier] = useState(data?.villeChantier || "");
  const [code_postal_chantier, setCodePostalChantier] = useState(
    data?.code_postal_chantier || "",
  );
  ...
  async function finalise_chantier() {
    ...
    if (
      ...
      !maitre_doeuvre ||
      !modele_maison ||
      adresse_du_chantier.trim() === "" ||
      villechantier.trim() === "" ||
      code_postal_chantier.trim() === ""
    ) {
    ...
    if (!clientID) {
      setError(
        "Erreur : Aucun client n'est associé. Veuillez retourner à la liste des clients.",
      );
      return;
    }
    ...
    <div className="BulleDuFormulaire">
      ...
      <form>
        ...
        <label>
          Maître d'oeuvre:
          <SearchableSelect
            options={liste_oeuvre}
            value={maitre_doeuvre}
            onChange={(value) => setMaitre_oeuvre(value)}
            getOptionValue={(maitre) => maitre.nomoe}
            getOptionLabel={(maitre) =>
              `${maitre.nommoe.trim()} ${maitre.prenommoe.trim()}`
            }
            placeholder="Choisir un maître d'oeuvre"
          />
        </label>
        <br />
        ...
        <label>
          ...
          <SearchableSelect
            options={modeles}
            value={modele_maison}
            onChange={(value) => setModeleMaison(value)}
        ...
        <label>
          ...
          <input
            type="text"
            placeholder="Adresse..."
            value={adresse_du_chantier}
            onChange={(e) => setAdresseDuChantier(e.target.value)}
          />
        </label>
        <br />
        ...
        <label>
          ...
          <input
            ...
            placeholder="Ville..."
            value={villechantier}
            onChange={(e) => setVilleChantier(e.target.value)}
          />
        </label>
        <br />
        ...
        <label>
          ...
          <input
            ...
            value={code_postal_chantier}
            onChange={(e) => setCodePostalChantier(e.target.value)}
          />
        </label>
        <br />

        <button type="button" onClick={finalise_chantier}>
